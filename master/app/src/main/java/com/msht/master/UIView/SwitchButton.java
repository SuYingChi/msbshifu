...
public class SwitchButton extends View{
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if(widthMode == MeasureSpec.UNSPECIFIED || widthMode == MeasureSpec.AT_MOST){
			widthSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 100, r.getDisplayMetrics());
	...
	public void draw(Canvas canvas) {
		...
		String testString = "接单中";
		TextPaint textPaint = new TextPaint();
		float rawSize = this.getRawSize(TypedValue.COMPLEX_UNIT_SP, 14);
		...
		if(toggleOn){
			...
			canvas.drawText(testString,getMeasuredWidth() / 2 -spotSize/2, baseline, textPaint);
		}else{
			testString="休息中";
